...
class OpenLibrary:
    ...
    def search_books_by_author(self, author, limit="20"):
        url = f"{self.base_url}/search.json?author={author}&limit={limit}"
        response = requests.get(url)
        return response.json()["docs"]
    ...
    def search_author(self, author, limit="5"):
        url = f"{self.base_url}/search/authors.json?q={author}&limit={limit}"
        print(url)
        response = requests.get(url)
        return response.json()["docs"]